...
const apartmentsColumns: ColumnDef<Apartment>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const building = row.getValue("building") as { name: string };
      return <p className="text-center font-medium text-sm">{building.name}</p>;
    },
  },
  {
    accessorKey: "price",
...
const ApartmentsPage = () => {
  ...
  const handleEdit = (updatedApartment: Apartment) => {
    setApartments((prev) =>
      prev.map((item) =>
        item.id === updatedApartment.id ? updatedApartment : item
      )
    );
  };
