...
class AddDefaultValuesToMapLocation1736975558600 {
    async up(queryRunner) {
        ...
        const combinedLandmarks = [...basicLandmarks, ...landmarks].map((landmark) => {
            const [lat, lng] = landmark.position;
            return MapLocations_model_1.MapLocations.create({
                name: landmark.name,
                type: landmark.type,
                lat: lat.toString(),
                lng: lng.toString(),
                classification: basicLandmarks.includes(landmark)
                    ? MapLocation_1.MapLocationClassification.primary
                    : MapLocation_1.MapLocationClassification.secondary,
            });
        });
        // Save all landmarks to the database
        await MapLocations_model_1.MapLocations.save(combinedLandmarks);
    }
...
exports.AddDefaultValuesToMapLocation1736975558600 = AddDefaultValuesToMapLocation1736975558600;
